# ASDF 피아노 리듬게임 — CLAUDE.md

## 프로젝트 요약

브라우저에서 바로 실행되는 한국어 ASDF/피아노 리듬게임이며, 서비스 노출명은 `리듬마스터`다.
...
- 업로드 차트는 곡 전체 후보를 만든 뒤 신뢰도 낮은 피치를 버리고, 곡 길이/복잡도/난이도에 맞춘 초당 노트 수 기준으로 고르게 추린다. 낮거나 높은 옥타브 음은 `도~높은 도` 범위의 가장 가까운 키로 접어 매핑한다.
